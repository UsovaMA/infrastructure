...
D_Heap::D_Heap(int d) {
  if (d <= 0)
    throw logic_error("Input error: invalid d.\n");
  this->d = d;
  size = -1;
...
void D_Heap::insert_group(Data **keys, int num) {
  if (size + num >= MAXsize)
    throw logic_error("Input error: too large set.\n");
  for (int i = 0; i < num; i++)
    key[i + size + 1] = keys[i];
  size += num;
  hilling();
...
Data* D_Heap::erase(int i) {
  if (isEmpty())
    throw logic_error("Heap is empty.\n");
  if ((i < 0) || (i > size))
    throw logic_error("Input error: invalid index.\n");
  Data* keys = key[i];
  if (i == size) {
    size--;
    return keys;
...
void D_Heap::transposition(int i, int j) {
  if ((i < 0) || (j < 0) || (i > size) || (j > size))
    throw logic_error("Input error: invalid indexes.\n");
  Data* tmp = key[i];
  key[i] = key[j];
  key[j] = tmp;
}
...
void PriorityQueue::add(Data **key) {
  heap->insert(key);
}
